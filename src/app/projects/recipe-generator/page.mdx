...
- **Tech stack:**  Discord bot, Cloudflare AI, whisper, Gemini. 
`,
  summary: `Recipe Generator is a Discord bot that turns cooking videos into consistent, structured recipes to make cooking while watching videos easier. It was built after testing common AI summaries that often hallucinated steps or failed when subtitles were missing. The bot accepts YouTube links in a personal Discord server, fetches subtitles when available, and otherwise transcribes audio using Cloudflare Whisper. It also pulls details from video descriptions and is prompted not to invent missing information, producing recipes with portion size, cook time, and clearly grouped ingredient lists in SI units. The system has two parts: a Discord bot that orchestrates the workflow and a transcriber that downloads audio, splits it into chunks, and transcribes in parallel for speed. The writeup shows a 60 percent reduction in processing time through parallel segmentation and discusses the minimum chunk length chosen to balance accuracy. It also documents a spec coding approach for generating the program from detailed prompts, plus a robust prompt template for recipe generation that enforces length limits and N A fallbacks. Finally, the page covers library failures in the YouTube ecosystem and how switching to more reliable tools avoided full rewrites. The result is a practical, free, and repeatable recipe extraction pipeline.`,
};
