...
<div className="flex justify-center w-full gap-5">
  <Image src="/images/projects/this-site/Pasted%20image%2020250820160328.png" width={348} height={255} alt='Image size difference' />
  <Image src="/images/projects/this-site/Picture3.png" width={257} height={255} alt='loading blur image sample'/>
</div>
...
`` will render a blur image when the actual image still loading.
...
Images are served as static files from `public/images/<route>/`, completely bypassing the webpack build graph. A build-time dimension manifest (`src/lib/image-dimensions.json`) provides `width` and `height` for every image, preventing layout shift without manual measurement.

The `Image` component from `next/image` is registered globally via `src/components/mdx.tsx`, so MDX files need zero boilerplate imports.

##### How it works

Images are organized under `public/images/` mirroring the app route structure:

```
public/images/
  hobbies/factorio/   ← images for /hobbies/factorio
  projects/this-site/ ← images for /projects/this-site
  dons-cookbook/       ← images for /dons-cookbook
```

The remark plugin converts `![[filename.png]]` into `<Image src="/images/<route>/filename.png" width={W} height={H} />` at build time, reading dimensions from the manifest.

##### Result

* No webpack processing of images — faster builds and lower memory usage.
* Obsidian-compatible `![[]]` syntax works seamlessly.
* Dimensions are injected automatically from the manifest.
...
The remark plugin parses the Markdown AST and transforms `![[filename]]` references into `<Image>` components with URL-based `src` attributes and auto-injected dimensions.

```md
![[color.png]]                          {/** ✅ Converted into <Image src="/images/.../color.png" /> */}
![[Pasted image 20250630155824.png]]    {/** ✅ Spaces are URL-encoded automatically */}
![Test](https://na.cx/i/zuXjaPX.jpg)   {/** ✅ Handled by default MDX logic */}
```

This approach keeps MDX files clean with Obsidian-compatible syntax, while `next/image` optimizations still apply.
