...
<span className='block dark:hidden'>![LeetCode Stats](https://leetcard.jacoblin.cool/DonChong?theme=dark&font=Archivo%20Narrow)</span>
<span className='hidden dark:block'>![LeetCode Stats](https://leetcard.jacoblin.cool/DonChong?theme=light&font=Archivo%20Narrow)</span>
...
### Bitmap
In LeetCode problems, we often need to count occurrences of alphabetical characters. A simple approach is to create an `number[]` and use the characters' ASCII values as indices. Since the ASCII codes for these characters span from **65** ('A') to **122** ('z'), we need an array of size **58** ($122 - 65 + 1 = 58$) to fit them all. Note that this range also includes non-alphabetical symbols such as `[`, `\`, `]`, `^`, and `_`.
```ts
...
```ts
let sum = new Int32Array(128) // will filled with 0 by itself
for(let char of "hello"){
...
2. Both `splice()` and `slice()` also run in O(N) time. While this is expected, use of these methods can still cause performance issues under tight computational constraints.

My solution to this two problem is using pointers.

**shift()**
For shift(), a better real world practice is to use prebuild packages like "yocto-queue". But in LeetCode, a simple implementation can be achieved by using a head pointer pointer the start of the array.
...
```
And the size of the array becomes:
```ts
arr.length - head;
```
...
**splice()` and `slice()**
In some questions like [224. Basic Calculator](https://leetcode.com/problems/basic-calculator/), we need to work on array recursively (at least in my approach). These two operations create a new array and will lead to run time exceeded error. Instead going back to basics primitive method, using two pointer to indicate start and end, and a pass by reference of the array would do the trick in O(1).
...
Depth-first search is usually implemented with recursion. Where we recur in a different order to achieve a different traversal order. 
```ts
...
1. Is Map in JavaScript deterministic?
2. Is insert/delete operations in Map O(1)

![[Pasted image 20260122195649.png]]
...
2. Both Insert or Delete are O(1) operations, regardless of the size. I use Gemini to generate a benchmark, which prove the O(1) characteristics.

Note: The max size of Map, or any object in V8 is 2^24, therefore I only tested up to 16777216.
...
Usually backtracking is recursion based, aims to trim off failed path and avoid forking multiple copy of data by modifying the steps. 

## Priority queue / Heap
It is a data structure where we only cares about the Largest element / Smallest element.
It is a complete binary tree presented as a array.
...
In LeetCode you can simply use them (or implement it like me if you have not done it before)
```typescript
